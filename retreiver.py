...
def get_system_prompt():
    return  """
    You are a machine learning model trained to extract key information from a financial text and provide an analysis. Given the input text, extract the following details and format them as a JSON object:

    1. **Total Monthly Deposits**: The total sum of all deposit transactions within the month. Use 0 if None
    2. **Total Monthly Withdrawals**: The total sum of all withdrawal transactions within the month. Use 0 if None
    3. **Regular Bills**: A list of recurring bills (e.g., rent, employee salary, utilities) with the amount and frequency.
    4. **Outstanding Loans**: Any outstanding loans with details like loan amount, lender, interest rate, borrower, etc.
    ...
    ### Extracted Details:
    {
        ...
        "Regular Bills": [
            {"type": "Rent", "amount": 0, "frequency": "Monthly"},
            {"type": "Employee Salary", "amount": 0, "frequency": "Monthly"},
            {"type": "Utilities", "amount": 0, "frequency": "Monthly"}
        ],
        "Outstanding Loans": [
            {"loan_amount": 0, "lender": "XYZ Bank", "borrower": "ABC Corp", "interest_rate": 5, "type": "Loan"}
        ],
    }

    Dont get creative with format, use the same format and field names as given in the example, with empty data if necessary. 
    """
# Use LangChain to combine document retrieval and answer generation
def generate_answer(file_type, file_name, loan_id):
     query = get_system_prompt()
